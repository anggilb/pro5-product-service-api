...
@Slf4j
@Component
public class StripeStrategySubscriptionUnsubscribe implements StripeStrategy {
    ...
    public StripeStrategySubscriptionUnsubscribe(SubscriptionRepository subscriptionRepository, SubscriptionMapper subscriptionMapper, StripeService stripeService, ProductProperties productProperties, UserService userService) {
    ...
    public boolean isApplicable(Event event) {
        return SUBSCRIPTION_UNSUBSCRIBE.getValue().equals(event.getType());
    }
    ...
    private Subscription processPlanChange(Subscription subscription) {
        Optional.of(subscription)
                ...
                .map(sub -> subscriptionMapper.mapSubscriptionToSubscriptionUnsubscribe(sub, subscription))
